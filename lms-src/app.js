...
(function () {
  ...
  function renderRateTable() {
    ...
    // 산출기간: 이수인원(A) 집계 범위. mode: all(전체) / round(차수) / date(이수일자)
    ...
    function periodLabel() {
      if (period.mode === 'round') {
        var f = period.from, t = period.to;
        if (!f && !t) return '전체 (차수 미지정)';
        return (f ? f + '차' : '처음') + ' ~ ' + (t ? t + '차' : '최신') + ' (차수 기준)';
      }
      if (period.mode === 'date') {
        var f2 = period.from, t2 = period.to;
        if (!f2 && !t2) return '전체 (기간 미지정)';
        return (f2 || '처음') + ' ~ ' + (t2 || '최신') + ' (이수일자 기준)';
      }
      return '전체';
    }
    var plabel = periodLabel();

    // 이수인원(A)은 산출기간 내 이수자만, 교육대상(분모)은 전체 기준
    var ppl = (state.result.persons || []).filter(function (p) { return p.기준정의; });
    var rows = DIRS.map(function (d) {
      var nvA = 0, exA = 0, target = 0;
      ppl.forEach(function (p) {
        if (p.직군정규화 !== d[0]) return;
        target++;
        if (!p.이수 || !inPeriod(p)) return;
        if (p.경력 === '신규자') nvA++; else if (p.경력 === '경력자') exA++;
      });
      var al = alloc[d[0]] || {}; var B = parseInt(al.배정) || 0, C = parseInt(al.채용) || 0;
      return { dir: d[0], 신규: nvA, 경력: exA, 소계: nvA + exA, 교육대상: target, 배정: B, 채용: C };
    });
    ...
    eb.onclick = function () {
      ...
      expBtnRows.forEach(function (r) { aoa.push([r.직군, r.신규자, r.경력자, r.이수소계, r.교육대상, r.배정인원, r.채용인원, r.대상대비, r.배정대비, r.채용대비]); });
      var ws = XLSX.utils.aoa_to_sheet(aoa); var wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, '이수율현황표'); XLSX.writeFile(wb, '이수율_현황표.xlsx');
    };
    head.appendChild(eb);
    ...
    var modeSel = el('select', { id: 'ratePeriodMode' });
    [['all', '전체'], ['round', '차수 기준'], ['date', '이수일자 기준']].forEach(function (o) { modeSel.appendChild(el('option', { value: o[0] }, o[1])); });
    modeSel.value = period.mode;
    modeSel.onchange = function () { period.mode = modeSel.value; saveAllocAndRender(); };
    pbar.appendChild(modeSel);
    if (period.mode === 'round') {
      var maxR = (state.coverage && state.coverage.summary && state.coverage.summary.최신차수) || '';
      var rf = el('input', { type: 'number', id: 'ratePeriodFrom', placeholder: '시작차수', style: 'width:100px' }); rf.value = period.from || '';
      var rt = el('input', { type: 'number', id: 'ratePeriodTo', placeholder: '끝차수', style: 'width:100px' }); rt.value = period.to || '';
      rf.onchange = function () { period.from = rf.value; saveAllocAndRender(); };
      rt.onchange = function () { period.to = rt.value; saveAllocAndRender(); };
      ...
      var df = el('input', { type: 'date', id: 'ratePeriodFrom' }); df.value = period.from || '';
      var dt = el('input', { type: 'date', id: 'ratePeriodTo' }); dt.value = period.to || '';
      df.onchange = function () { period.from = df.value; saveAllocAndRender(); };
      dt.onchange = function () { period.to = dt.value; saveAllocAndRender(); };
    ...
    hd.innerHTML = '(이수인원) 총 <b>' + fmt(tot.소계) + '명</b> (전담사회복지사 ' + fmt(rows[0].소계) + '명, 생활지원사 ' + fmt(rows[1].소계) + '명) · 산출기간: <b>' + plabel + '</b>';
    card.appendChild(hd);
